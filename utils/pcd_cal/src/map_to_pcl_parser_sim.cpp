...
public:
  PCDPublisher() : Node("pcd_publisher") {
    ...
    this->declare_parameter<bool>("flip_flag", true);
    ...
    this->declare_parameter<std::string>("graph_filename", "/root/ros2_ws/src/Isaac_ml/map/map_vertex_test.yaml");
    ...
    graph_filename = this->get_parameter("graph_filename").as_string();
    same_length_flag = this->get_parameter("same_length_flag").as_bool();
    flip_flag = this->get_parameter("flip_flag").as_bool();
    ...
    RCLCPP_INFO(this->get_logger(), "graph_filename: %s", graph_filename.c_str());
    RCLCPP_INFO(this->get_logger(), "same_length_flag: %s", same_length_flag ? "true" : "false");
    RCLCPP_INFO(this->get_logger(), "flip_flag: %s", flip_flag ? "true" : "false");
    ...
    publisher_pcd_tree_line_segments = this->create_publisher<sensor_msgs::msg::PointCloud2>("point_cloud_tree_line_segments", 10);
    publisher_graph_marker = this->create_publisher<visualization_msgs::msg::Marker>("graph_marker", 10);
...
private:
  void publishPointCloud() {
    ...
    if (point_cloud_tree_clustered->size ()) {
      ...
      sensor_msgs::msg::PointCloud2 output10;
      pcl::toROSMsg(*endpoint_squares, output10);
      output10.header.frame_id = "map";
      output10.header.stamp = this->get_clock()->now();
      publisher_pcd_tree_line_segments->publish(output10);
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr filter_line_by_endpoints(pcl::PointCloud<pcl::PointXYZ>::Ptr line_points, double threshold)
  {
    // line_points를 선의 시작부터 끝으로 정렬했다고 가정합니다.
    // 양쪽 끝에서 각각 삭제할 인덱스를 찾습니다.
    int start_idx = 0;
    int end_idx = static_cast<int>(line_points->points.size()) - 1;

    // 왼쪽(시작점)에서 순차적으로 검사
    for (int i = 0; i < static_cast<int>(line_points->points.size()) - 1; ++i) {
      double dist = squaredDistance(line_points->points[i], line_points->points[i + 1]);
      if (dist > threshold) {
        // 이 시점 이후는 잘라냅니다.
        start_idx = i + 1;
      } else {
    ...
    // 오른쪽(끝점)에서 순차적으로 검사
    for (int i = static_cast<int>(line_points->points.size()) - 1; i > 0; --i) {
      double dist = squaredDistance(line_points->points[i], line_points->points[i - 1]);
      if (dist > threshold) {
        end_idx = i - 1;
      } else {
    ...
    // start_idx와 end_idx 사이의 점들만 남깁니다.
    pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_line(new pcl::PointCloud<pcl::PointXYZ>);
    for (int i = start_idx; i <= end_idx; ++i) {
      filtered_line->push_back(line_points->points[i]);
    }
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr find_grid(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr tree_points, pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_cloud) {
    ...
    if (flip_flag) {
      // x axis is close to eigen vector
      ...
      // y axis is close to eigen vector
      coordinate_origin_min = intersections[0]; //(x1, y1)
    ...
    for (auto& pair : cloudsById_grid) {
      Eigen::Vector3f direction = new_curve_on_grid[pair.first].eigen_vector;
      
      // cloud는 unordered_map의 값, 즉 pcl::PointCloud<pcl::PointXYZ>::Ptr 내의 points 벡터입니다.
      std::sort(pair.second->points.begin(), pair.second->points.end(),
        [&direction](const pcl::PointXYZ& a, const pcl::PointXYZ& b) {
          // 각 점을 line의 방향에 투영한 값을 기준으로 정렬합니다.
          float projection_a = a.x * direction[0] + a.y * direction[1] + a.z * direction[2];
          float projection_b = b.x * direction[0] + b.y * direction[1] + b.z * direction[2];
          return projection_a < projection_b;
        });
    }

    for (auto& pair : cloudsById_grid) {
      pair.second = filter_line_by_endpoints(pair.second, cluster_tolerance);
    }

    pcl::PointCloud<pcl::PointXYZI>::Ptr mergedCloud(new pcl::PointCloud<pcl::PointXYZI>);
    std::vector<std::pair<pcl::PointXYZ, pcl::PointXYZ>> line_segments;
    ...
    if (same_length_flag) {
      ...
      for (auto& pair : cloudsById_grid) {
        ...
        line_segments.push_back(std::make_pair(start_point, end_point));
    ...
    *point_interpolation_merged_xyz += *tree_points;
    ...
    std::vector<std::pair<int,int>> manual_edges;
    endpoint_squares = createAlignedRectanglesCloud(line_segments, manual_edges);
    auto edges = buildPointGraph(
    endpoint_squares,
    ...
    4.0,
    grid_resolution,
    grid_2d.info.origin.position.x, grid_2d.info.origin.position.y);
    endpoint_squares->width  = endpoint_squares->points.size();
    endpoint_squares->height = 1;
    endpoint_squares->is_dense = true;
    publishGraphMarkers(edges, endpoint_squares, publisher_graph_marker);

    return mergedCloud;
  }
  
  /**
   * @brief 선분의 양 끝점에서 회전된 정사각형 코너 점들을 하나의 PointCloud로 생성합니다.
   *        정사각형 한 변의 길이는 선분들 중점 간 평균 거리의 2배입니다.
   * @param line_segments  시작점·끝점 쌍 벡터
   * @return 생성된 모든 사각형 코너 점들을 포함한 PointCloud<pcl::PointXYZ>::Ptr
   */
  pcl::PointCloud<pcl::PointXYZ>::Ptr createEndpointSquaresCloud(
      const std::vector<std::pair<pcl::PointXYZ, pcl::PointXYZ>>& line_segments) {
    std::vector<Eigen::Vector2f> centers;
    centers.reserve(line_segments.size());
    for (const auto& seg : line_segments) {
      centers.emplace_back((seg.first.x + seg.second.x) * 0.5f,
                           (seg.first.y + seg.second.y) * 0.5f);
    }
    std::vector<float> gaps;
    for (size_t i = 1; i < centers.size(); ++i) {
      gaps.push_back((centers[i] - centers[i-1]).norm());
    }
    float avg_gap = 0.0f;
    if (!gaps.empty()) {
      avg_gap = std::accumulate(gaps.begin(), gaps.end(), 0.0f) / gaps.size();
    }
    float half_side = avg_gap;

    auto cloud = pcl::make_shared<pcl::PointCloud<pcl::PointXYZ>>();
    cloud->is_dense = true;

    for (const auto& seg : line_segments) {
      Eigen::Vector2f vec(seg.second.x - seg.first.x,
                          seg.second.y - seg.first.y);
      float len = vec.norm();
      if (len < 1e-6f) continue;
      Eigen::Vector2f dir = vec / len;
      Eigen::Vector2f perp(-dir.y(), dir.x());

      for (const auto& pt : {seg.first, seg.second}) {
        std::array<Eigen::Vector2f,4> offs = {
          dir * half_side + perp * half_side,
          dir * half_side - perp * half_side,
          -dir * half_side - perp * half_side,
          -dir * half_side + perp * half_side
        };
        for (auto& o : offs) {
          cloud->points.emplace_back(pt.x + o.x(), pt.y + o.y(), pt.z);
        }
        cloud->points.emplace_back(cloud->points.back());
      }
    }
    cloud->width = cloud->points.size();
    cloud->height = 1;
...
/**
 * @brief 전체 선분 벡터의 평균으로 주(line) 방향을 계산하고,
 *        이 방향의 수직 벡터(perp_dir)를 따라 인접 선분 간 gap으로 사각형을 생성합니다.
 *        모든 사각형은 같은 perp_dir 방향으로 일렬로 정렬됩니다.
 * @param line_segments 시작점·끝점 쌍 벡터
 * @return 일렬로 정렬된 사각형 코너 점들을 포함한 PointCloud<pcl::PointXYZ>::Ptr
 */
pcl::PointCloud<pcl::PointXYZ>::Ptr createAlignedRectanglesCloud(
    const std::vector<std::pair<pcl::PointXYZ, pcl::PointXYZ>>& line_segments,
    std::vector<std::pair<int,int>>& manual_edges) {
  std::vector<Eigen::Vector2f> mids;
  manual_edges.clear();

  mids.reserve(line_segments.size());
  for (const auto& seg : line_segments) {
    mids.emplace_back((seg.first.x + seg.second.x) * 0.5f,
                      (seg.first.y + seg.second.y) * 0.5f);
  }
  Eigen::Vector2f main_dir(0, 0);
  for (const auto& seg : line_segments) {
    Eigen::Vector2f v(seg.second.x - seg.first.x,
                      seg.second.y - seg.first.y);
    float l = v.norm(); if (l < 1e-6f) continue;
    main_dir += v / l;
  }
  if (main_dir.norm() < 1e-6f) main_dir = Eigen::Vector2f(1, 0);
  else main_dir.normalize();
  Eigen::Vector2f perp_dir(-main_dir.y(), main_dir.x());

  size_t furthest = 0; float max_proj = -FLT_MAX;
  for (size_t i = 0; i < mids.size(); ++i) {
    float proj = mids[i].dot(main_dir);
    if (proj > max_proj) { max_proj = proj; furthest = i; }
  }
  if (mids.size() > 1) {
    size_t prev = (furthest == 0 ? 1 : furthest - 1);
    Eigen::Vector2f diff = mids[furthest] - mids[prev];
    if (diff.dot(perp_dir) < 0) perp_dir = -perp_dir;
  }

  auto cloud = pcl::make_shared<pcl::PointCloud<pcl::PointXYZ>>();
  cloud->is_dense = true;
  for (size_t i = 0; i + 1 < line_segments.size(); ++i) {
    const auto& seg = line_segments[i];
    float gap = std::abs((mids[i+1] - mids[i]).dot(perp_dir));
    pcl::PointXYZ p0 = seg.first;
    pcl::PointXYZ p1 = seg.second;
    pcl::PointXYZ p2(p1.x + perp_dir.x() * gap / 2.0f,
                    p1.y + perp_dir.y() * gap / 2.0f,
                    p1.z);
    pcl::PointXYZ p3(p0.x + perp_dir.x() * gap / 2.0f,
                    p0.y + perp_dir.y() * gap / 2.0f,
                    p0.z);
    pcl::PointXYZ p4(p2.x + main_dir.x() * gap / 2.0f,
                    p2.y + main_dir.y() * gap / 2.0f,
                    p2.z);
    pcl::PointXYZ p5(p3.x - main_dir.x() * gap / 2.0f,
                    p3.y - main_dir.y() * gap / 2.0f,
                    p3.z);
    if (i == 0) {
      pcl::PointXYZ p6(p4.x - perp_dir.x() * gap,
                      p4.y - perp_dir.y() * gap,
                      p4.z);
      pcl::PointXYZ p7(p5.x - perp_dir.x() * gap,
                      p5.y - perp_dir.y() * gap,
                      p5.z);
      cloud->points.push_back(p6);
      int idx6 = cloud->points.size()-1;
      cloud->points.push_back(p7);
      int idx7 = cloud->points.size()-1;
      manual_edges.emplace_back(idx6, idx7);
    }
    cloud->points.push_back(p4);
    int idx4 = cloud->points.size()-1;
    cloud->points.push_back(p5);
    int idx5 = cloud->points.size()-1;
    manual_edges.emplace_back(idx4, idx5);
    if (i == line_segments.size() - 2) {
      pcl::PointXYZ p6(p4.x + perp_dir.x() * gap,
                      p4.y + perp_dir.y() * gap,
                      p4.z);
      pcl::PointXYZ p7(p5.x + perp_dir.x() * gap,
...
/**
 * @brief ROS2 Marker 메시지로 RViz에서 LINE_LIST로 시각화
 */
void publishGraphMarkers(
  ...
  for (auto& e : edges) {
    ...
    p1.x = cloud->points[e.first].x;
    p1.y = cloud->points[e.first].y;
    p1.z = cloud->points[e.first].z;
    p2.x = cloud->points[e.second].x;
    p2.y = cloud->points[e.second].y;
    p2.z = cloud->points[e.second].z;
  ...
  float origin_x, float origin_y)
{
  ...
  int pix_i_x = std::floor((Pi.x - origin_x)/resolution);
  int pix_i_y = std::floor((Pi.y - origin_y)/resolution);
  int pix_j_x = std::floor((Pj.x - origin_x)/resolution);
  int pix_j_y = std::floor((Pj.y - origin_y)/resolution);
  float dx = pix_i_x - pix_j_x;
  float dy = pix_i_y - pix_j_y;
  float dist_pix = std::sqrt(dx*dx + dy*dy);


  if (dist_pix <= pixel_thresh) {
    edges_out.emplace_back(i,j);
    adj[i].push_back(j);
    adj[j].push_back(i);
  } else {
    // 중간점 생성
    pcl::PointXYZ mid;
    mid.x = 0.5f*(Pi.x + Pj.x);
    mid.y = 0.5f*(Pi.y + Pj.y);
    mid.z = 0.5f*(Pi.z + Pj.z);

    cloud->points.push_back(mid);  
    int k = static_cast<int>(cloud->points.size()) - 1;

    // 재귀 분할
    subdivideEdge(i, k, cloud, adj, edges_out, pixel_thresh, resolution, origin_x, origin_y);
    subdivideEdge(k, j, cloud, adj, edges_out, pixel_thresh, resolution, origin_x, origin_y);
  }
}

/**
 * @brief 포인트 클라우드→맵 픽셀 기준으로 graph 생성, 엣지 분할, YAML 저장
 */
std::vector<std::pair<int,int>> buildPointGraph(
    pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,
    float world_radius,
    const std::string& yaml_filename,
    const std::vector<std::pair<int,int>>& manual_edges,
    float pixel_thresh,
    float resolution,
    float origin_x, float origin_y)
{
  // 1) KD-트리로 world_radius 내 자동 엣지 생성
  pcl::KdTreeFLANN<pcl::PointCloud<pcl::PointXYZ>::PointType> kdtree;
  kdtree.setInputCloud(cloud);
  std::vector<std::pair<int,int>> raw_edges;
  for (size_t i = 0; i < cloud->points.size(); ++i) {
    std::vector<int> idx; std::vector<float> dist;
    if (kdtree.radiusSearch(cloud->points[i], world_radius, idx, dist) > 1) {
      for (size_t kk = 1; kk < idx.size(); ++kk) {
        int j = idx[kk];
        if (j > static_cast<int>(i)) raw_edges.emplace_back(i,j);
      }
    }
  }
  // 2) subdivide + manual 엣지 추가
  std::map<int,std::vector<int>> adj;
  std::vector<std::pair<int,int>> final_edges;
  // 자동 엣지
  for (auto &e : raw_edges) {
    subdivideEdge(e.first, e.second, cloud, adj, final_edges,
                  pixel_thresh, resolution, origin_x, origin_y);
  }
  // 수동 엣지
  for (auto [i,j] : manual_edges) {
    if (i>j) std::swap(i,j);
    if (std::find(adj[i].begin(), adj[i].end(), j)==adj[i].end()) {
      subdivideEdge(i, j, cloud, adj, final_edges,
                    pixel_thresh, resolution, origin_x, origin_y);
    }
  }
  // 3) YAML 저장 (픽셀 좌표로)
  std::ofstream out(yaml_filename);
  out << "graph:\n  vertices:\n";
  for (size_t v = 0; v < cloud->points.size(); ++v) {
    const auto &P = cloud->points[v];
    int px = std::floor((P.x - origin_x)/resolution);
    int py = std::floor((P.y - origin_y)/resolution);
    out << "    " << v << ":\n"
        << "      x: " << px << "\n"
        << "      y: " << py << "\n"
        << "      connections: [";
  ...
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr publisher_pcd_tree_line_segments;
  rclcpp::Publisher<nav_msgs::msg::OccupancyGrid>::SharedPtr publisher_2d_occupancy_grid;
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr publisher_graph_marker;
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr endpoint_squares;
};
