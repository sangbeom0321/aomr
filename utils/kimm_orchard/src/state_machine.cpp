...
public:
    ControlInputNode() : Node("control_input_node") {
        ...
        local_path_sub_ = this->create_subscription<nav_msgs::msg::Path>(
            "local_path", 10, std::bind(&ControlInputNode::localPathCallback, this, std::placeholders::_1));
        ...
        current_global_index_ = 0;  // 글로벌 경로에서 현재 위치와 가장 가까운 포인트의 인덱스
    }
...
private:
    ...
    void baseLinkOdomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        if ((odom_cnt_ % skipping_hz_) == 0) {
            ...
            RCLCPP_INFO(this->get_logger(), "last_goal_point_: %f", last_goal_point_.position.x);
            if (goal_point_.position.x == 0.0) {
    ...
    void updateControlMode() {
        ...
        } else if (distance_to_goal < 0.5 && current_mode_ != 3) {
    ...
    void localPathPublish() {
        if (!local_path_.poses.empty()) {
            // 현재 위치에서 가장 가까운 포인트 찾기
            int closest_index = findClosestIndex(local_path_, current_pose_);
            int global_index = findPointInPath(local_path_.poses[closest_index].pose, global_path_);
            int index_diff = global_index - closest_index;

            if (global_index == -1) {
                // 새로운 로컬 경로 생성 (가장 가까운 포인트부터 앞으로 100개 포인트)
                nav_msgs::msg::Path forward_path;
                forward_path.header = local_path_.header;
                
                // 가장 가까운 포인트부터 앞으로 최대 100개 포인트 추가
                size_t end_index = std::min(static_cast<size_t>(closest_index + 100), local_path_.poses.size());
                for (size_t i = closest_index; i < end_index; ++i) {
                    forward_path.poses.push_back(local_path_.poses[i]);
                }
                
                // 마지막 포인트가 2개 미만이면 마지막 2개 포인트 사용
                if (forward_path.poses.size() < 2 && local_path_.poses.size() >= 2) {
                    forward_path.poses.clear();
                    forward_path.poses.push_back(local_path_.poses[local_path_.poses.size() - 2]);
                    forward_path.poses.push_back(local_path_.poses[local_path_.poses.size() - 1]);
                }
                
                // 로컬 경로 발행
                local_path_pub_->publish(forward_path);
            }
            else {
                current_global_index_ = std::max(global_index, current_global_index_);
                nav_msgs::msg::Path forward_path;
                forward_path.header = local_path_.header;

                int global_to_local_index_ = current_global_index_ - index_diff;
                
                // 가장 가까운 포인트부터 앞으로 최대 100개 포인트 추가
                size_t end_index = std::min(static_cast<size_t>(global_to_local_index_ + 100), local_path_.poses.size());
                for (size_t i = global_to_local_index_; i < end_index; ++i) {
                    forward_path.poses.push_back(local_path_.poses[i]);
                }
    ...
    int findPointInPath(const geometry_msgs::msg::Pose& point, const nav_msgs::msg::Path& path, double tolerance = 0.01) {
        for (size_t i = 0; i < path.poses.size(); ++i) {
            double dx = point.position.x - path.poses[i].pose.position.x;
            double dy = point.position.y - path.poses[i].pose.position.y;
            double distance = std::hypot(dx, dy);
    ...
    nav_msgs::msg::Path forward_local_path_;
